...
public class KDTreeVisualization extends Component{
  ...
  private TreeNode newNode(LinkedList<Point> pointList, int depth) {
        if(!pointList.isEmpty()){
            int a = depth % 2;
            java.util.Comparator<Point> c = new PointComparator(a);
  ...
  private Point listSearchNN(Point p){
    //to be implemented
      ...
      for (int i=0; i<points.size(); i++) {
          ...
          if (p != points.get(i)){break;}
          distance = getDistance(p, i);
      ...
      return points.get(IndexBest);
  }

  private double getDistance(Point p, int index){
      //pythagoras
      double  distance = Math.sqrt(Math.pow((Math.abs(p.x)-Math.abs(points.get(index).x)), 2) + Math.pow(Math.abs(p.y)-Math.abs(points.get(index).y), 2));
      return distance;
  }
  ...
  private Point treeSearchNN(Point p){
    //to be implemented
      return kdSearcher(kdRoot, p, null, 0).position;
  }
  private TreeNode kdSearcher(TreeNode root, Point p, TreeNode best, int depth) {
      ...
      if (p.distance(root.position) < p.distance(best.position))
            best = root;

      int a = depth % 2;

      TreeNode child;
      if (a == 0 && p.x < root.position.x || a == 1 && p.y < root.position.y) child = root.left;
      else child = root.right;

      best = kdSearcher(child, p, best, depth + 1);


      double distA;
      if (a == 0) distA = Math.abs(root.position.x - p.x);
      else distA = Math.abs(root.position.y - p.y);

      if (a == 1 && p.y < root.position.y || a == 0 && p.x < root.position.x ) {
          child = root.right;
      }
      else {
          child = root.left;
      ...
      if (distA <= p.distance(best.position)) {
            best = kdSearcher(child, p, best, depth + 1);
      }
