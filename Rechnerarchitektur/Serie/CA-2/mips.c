...
void mips_add(Instruction *instruction) {
	...
	InstructionTypeR TypeR = instruction->r;
	registers[TypeR.rd] = (signed) registers[TypeR.rs] + (signed) registers[TypeR.rt];
}

/* ADDI */
void mips_addi(Instruction *instruction) {
	/* TODO: Task (e) implement ADDI here */
	InstructionTypeI TypeI = instruction->i;
	registers[TypeI.rt] = (signed)registers[TypeI.rs] + (signed)signExtend(TypeI.immediate);
}

/* JAL */
void mips_jal(Instruction *instruction) {
	/* TODO: Task (e) implement JAL here */
	InstructionTypeJ TypeJ = instruction->j;
	registers[31] = pc;
	pc = (pc & 0xf0000000) | (TypeJ.address << 2);
}
...
void mips_lui(Instruction *instruction) {
	...
	InstructionTypeI TypeI = instruction->i;
	registers[TypeI.rt]=TypeI.immediate << 16;
}
...
void mips_sw(Instruction *instruction) {
	...
	InstructionTypeI TypeI = instruction->i;
	storeWord(registers[TypeI.rt], registers[TypeI.rs] + (signed)signExtend(TypeI.immediate));
}
