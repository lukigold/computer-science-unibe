...
typedef struct {  unsigned immediate:16;
                  unsigned rt:5;
                  unsigned rs:5;
                  unsigned opcode:6; } InstructionTypeI;

typedef struct {  unsigned adress:26;
                  unsigned opcode:6; } InstructionTypeJ;

typedef struct {  unsigned funct:6;
                  unsigned shamt:5;
                  unsigned rd:5;
                  unsigned rt:5;
                  unsigned rs:5;
                  unsigned opcode:6; } InstructionTypeR;


/* TODO Task (d) add union Instruction here */

typedef union { InstructionTypeI i;
                InstructionTypeJ j;
                InstructionTypeR r; } Instruction;
...
typedef struct {  char name[OP_NAME_LENGTH+1];
                  InstructionType type;
                  void (*operation)(Instruction*); } Operation;

/* TODO Task (g) add structure Function here */

typedef struct {  char name[FUNC_NAME_LENGTH+1];
                  void (*function)(Instruction*); } Function;
...
Function f = functions[i->r.funct];
  ...
  switch (o.type) {
    ...
    case rType:
                  printf("%-4s %02d, %02d, %02d, 0x%04x \n", f.name, i->r.rd, i->r.rs, i->r.rt, i->r.shamt); /* %#06x does not work for some reason*/
                  break;
